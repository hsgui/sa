 ...
 * with redis to save the result -- mutual information
 * @author hsgui
 *
 */

@Service
public class SentimentalWordExpander3 {
	...
	public static void main(String[] args){
		...
		SentimentalWordExpander3 expander = new SentimentalWordExpander3();
	...
	*/
	...
	public void initEnvironment(){
		...
		try{
			...
			logger.info("initEnvironment -- connect to server ip: {}, port: {}",ip, port);
	...
	public void updateContextIndication(HashMap<String, Double> contextIndication, String neighborWord, int wordCount, int contextCountInWord, int contextCountInTotal){
		...
		redis.hset(this.seedContextFileName, neighborWord, newValue+"");
		/*
	...
	public void updateWordIndication(HashMap<String, Double> wordIndication, String word, String neighborWord, int contextCount, int wordCountInContext, int wordCountInTotal){
		...
		redis.hset(this.candidateWordFileName, word, newValue+"");
		/*
	...
	public HashMap<String, Double> filterContextList(HashMap<String, Double> map){
		HashMap<String, Double> filteredMap = new HashMap<String, Double>(map.size());
	...
	public void runAll(HashSet<String> seeds){
		...
		this.contextIndication = this.filterContextList(this.contextIndication);
		array = sortHashMap(this.contextIndication);
		int contextListSize = array.size();
		for(int contextIndex = 0; contextIndex < contextListSize; contextIndex++){	//for every context
			logger.info("runAll -- contextIndex: {}, contextListSize: {}", contextIndex, contextListSize);
	...
	public <T extends Number> ArrayList<Entry<String, T>> sortHashMap(HashMap<String, T> hashMap){
		...
		Collections.sort(array, new Comparator<Entry<String, ? extends Number>>(){
				public int compare(Entry<String, ? extends Number> o1, Entry<String, ? extends Number> o2){
					return (o2.getValue().doubleValue()-o1.getValue().doubleValue() > 0)?1:-1;
				}
		});
		return array;
	}
	...
	public HashMap<String, Integer> extractNeighborWords(String word){
		...
		if (wordsOccurrence.get(word) == null)	{
			wordsOccurrence.put(word, count);
			redis.hset(this.wordCountInTotalFileName, word, count+"");
		}
		Date end = new Date();
	...
	public HashMap<String, Integer> extractCandidateWords(String neighborWord){
		...
		if (contextOccurrence.get(neighborWord) == null) {
			contextOccurrence.put(neighborWord, count);
			redis.hset(this.neighborCountInTotalFileName, neighborWord, count+"");
		}
